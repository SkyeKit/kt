...
export type StatusId =
  ...
  // ④ 暗港（Underdocks）专属机制（Underdocks.md §4）
  | 'smoggy' // 烟雾弥漫：玩家每回合只能打出 1 张技能牌（活雾专属）
  | 'plating' // 覆甲：回合结束时获得等量格挡，回合开始时层数减 1（下水道蚌、乐加维林族母）
  | 'hard_shell' // 硬化外壳：每回合失去的生命值不超过 X（鬼祟珊瑚群）
  | 'suck' // 吮吸：每次造成未被格挡的伤害时获得 X 点力量（化石追踪者）
  | 'shriek' // 尖叫：生命值降至 X 时强制击晕一回合（骇鳗）
  | 'steam' // 蒸汽喷发：累计层数，生命值归零时首次不死亡，下回合自爆造成等量伤害（瀑布巨兽）
  | 'timid' // 胆小：每回合开始时获得等量格挡（花园幽灵鳗）
